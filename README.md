...
### 1. 회원 가입(Sign up) 및 토큰(Token) 관리   
   ...
   **1.3 토큰(크레딧)**  
   - 사용자는 충전/결제 시스템을 통해(미구현)을 통해 토큰(크레딧)을 획득할 수 있어야 한다.
...
### 2. 여행 계획(Plan) 생성, 수정, 삭제    
   ...
   **2.4 @@@@@공개/비공개 설정 또는 여행 계획 공개**    
   - 계획은 다른 사용자와 여행 계획 공유 게시판에서 공유된다.
...
### 4. 사용자 팔로우/언팔로우(Follow/Unfollow)    
   ...
   **4.3 @@@@@ 따로 대시보드가 있어야 할듯 팔로우 계획 모니터링**  팔로우 대시보드 별도
   - 팔로우한 계획은 별도 대시보드에서 확인할 수 있어야 한다.

### 5. 알림(Notification) 기능    
   **5.1 계획 생성 알림**  
   - 팔로우한 사용자가 계획을 생성할 경우, 내게 사용자에게 시스템에서 알림을 발송한다.
