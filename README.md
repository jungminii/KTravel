<img src="https://github.com/KT-HOO/KTravel/blob/main/img/Ktravel.png" width="350" height="270"/>
...
1. **회원 가입(Sign up) 및 토큰(Token) 관리**   
   1.1 **회원 가입 및 로그인**  
   - 사용자는 이메일 또는 소셜 계정을 통해 회원가입 및 로그인이 가능해야 한다.
   
   1.2 **사용자 프로필 관리**  
   - 사용자는 자신의 프로필 정보를 수정할 수 있어야 한다.
   
   1.3 **토큰(크레딧)**  
   - 사용자는 여행 계획 생성, 팔로우, 추천 등을 통해 토큰(크레딧)을 획득하거나 사용할 수 있어야 한다.
   - 토큰 획득 및 사용 기록이 사용자 대시보드에서 조회 가능해야 한다.
   - 토큰은 여행 계획 추천 기능에 사용될 수 있어야 한다.

2. **여행 계획(Plan) 생성, 수정, 삭제**    
   2.1 **여행 계획 생성**  
       - 사용자는 목적지, 날짜, 활동 등의 상세 내용을 포함한 여행 계획을 생성할 수 있어야 한다.
   
   2.2 **여행 계획 수정 및 삭제**  
       - 사용자는 이미 생성한 여행 계획을 수정하거나 삭제할 수 있어야 한다.
   
   2.3 **공개/비공개 설정**  
       - 사용자는 각 여행 계획에 대해 공개 여부를 설정할 수 있어야 하며, 공개된 계획은 다른 사용자와 공유됩니다.

3. **다른 사용자의 계획 팔로우/언팔로우(Follow/Unfollow)**    
   3.1 **팔로우 기능**  
       - 사용자는 다른 사용자가 만든 여행 계획을 팔로우하여 관심 있는 계획의 업데이트를 실시간으로 받을 수 있습니다.
   
   3.2 **언팔로우 기능**  
       - 더 이상 관심이 없는 사용자의 계획을 언팔로우할 수 있습니다.
   
   3.3 **팔로우된 계획 모니터링**  
       - 팔로우한 계획의 변경 사항이나 새로운 업데이트는 사용자에게 알림으로 제공됩니다.

4. **추천(Recommendation) 시스템**    
   4.1 **사용자 맞춤형 추천**  
       - 사용자의 여행 취향(예: 방문한 지역, 좋아하는 활동 등)을 분석하여 맞춤형 여행 계획을 추천합니다.
   
   4.2 **트렌드 기반 추천**  
       - 현재 플랫폼에서 인기 있는 여행 계획이나 활동을 추천하는 기능을 포함합니다.
   
   4.3 **사용자 활동 분석**  
       - 사용자들이 팔로우한 계획이나 조회 기록을 분석하여 관련 여행 계획을 추천합니다.

5. **알림(Notification) 기능**    
   5.1 **실시간 알림**  
       - 사용자가 팔로우한 여행 계획에 변경 사항이 있을 경우 알림을 받을 수 있습니다.
   
   5.2 **계획 마감 알림**  
       - 사용자의 여행 계획이 곧 시작되거나 마감 기한이 가까워질 때 알림을 받을 수 있습니다.
   
   5.3 **추천 여행 알림**  
       - 추천 시스템에서 새로운 맞춤형 여행 계획이 있을 경우 사용자에게 알림이 전송됩니다.
...
1. **트랜잭션 일관성 유지**    
   1.1 **데이터 일관성**  
       - 사용자의 토큰 사용, 여행 계획 생성/수정/삭제 등의 트랜잭션이 중단되지 않고 일관성을 유지해야 합니다. 분산 트랜잭션 처리 메커니즘을 통해 이를 보장합니다.
   
   1.2 **ACID 속성 보장**  
       - 중요한 데이터(회원 정보, 여행 계획, 토큰 내역 등)에 대해 ACID 속성을 보장하는 데이터베이스 구조를 구현합니다.

2. **서비스 간 느슨한 결합**    
   2.1 **마이크로서비스 아키텍처**  
       - 각 기능이 독립된 마이크로서비스로 운영되며, 서로 간의 결합도를 최소화하여 유지보수 및 확장성을 높입니다.
   
   2.2 **API 기반 통신**  
       - 마이크로서비스 간의 통신은 RESTful API 또는 메시지 큐 기반으로 이루어져야 하며, 서비스의 장애나 업데이트가 다른 서비스에 영향을 미치지 않도록 설계됩니다.

3. **장애 격리**    
   3.1 **장애 감지 및 복구**  
       - 특정 서비스가 실패하더라도 전체 시스템이 영향을 받지 않도록 장애 격리 메커니즘을 구현합니다.
       - 개별 서비스 장애 시 자동 복구 및 롤백이 가능해야 합니다.
   
   3.2 **모니터링 및 로그 관리**  
       - 실시간 모니터링을 통해 장애를 감지하고, 이를 분석하여 빠르게 대응할 수 있는 로그 관리 시스템을 구축합니다.

4. **확장성 및 유연성**    
   4.1 **자동 확장**  
       - 사용자 수 증가에 따라 자동으로 서버 리소스를 확장할 수 있는 기능을 갖추어야 합니다. 이를 위해 컨테이너화된 서비스나 클라우드 인프라를 사용할 수 있습니다.
   
   4.2 **유연한 데이터베이스 확장**  
       - 데이터베이스는 수평적 확장을 지원하며, 많은 양의 사용자 데이터를 효율적으로 처리할 수 있어야 합니다.
   
   4.3 **플랫폼 확장**  
       - 새로운 기능이 추가되더라도 기존 시스템에 영향을 주지 않고 손쉽게 확장할 수 있는 유연한 아키텍처를 적용해야 합니다.

5. **보안 및 개인정보 보호**    
   5.1 **데이터 암호화**  
       - 사용자 정보 및 민감한 데이터는 저장 및 전송 시 모두 암호화됩니다.
   
   5.2 **접근 제어**  
       - 각 서비스에 대한 접근 권한을 엄격히 관리하며, 사용자 인증 및 권한 부여를 통해 보호합니다.
   
   5.3 **개인정보 보호**  
       - 사용자의 여행 계획과 같은 개인정보는 외부에 노출되지 않도록 정책을 설정하고 준수합니다.


3. : 사용자는 이메일 또는 소셜 계정을 통해 회원가입 및 로그인이 가능해야 한다. 
4. 사용자 프로필 관리: 사용자는 자신의 프로필 정보를 수정할 수 있어야 한다.
5. 예약과 동시에 결제가 진행된다.
6. 예약이 되면 예약 내역(Message)이 전달된다.
7. 고객이 예약을 취소할 수 있다.
8. 예약 사항이 취소될 경우 취소 내역(Message)이 전달된다.
9. 숙소에 후기(review)를 남길 수 있다.
10. 전체적인 숙소에 대한 정보 및 예약 상태 등을 한 화면에서 확인 할 수 있다.(viewpage)
